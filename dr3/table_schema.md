...
* Generally we do not recommend values found only in the `main_allspec` catalogue as these are only for expert use and are for diagnostic purposes.

We have shortened the catalogue filenames to save space (remove the `GALAH_DR3` and `v2`).
...
| `dr2_radial_velocity_error` | `VAC_rv`, `VAC_GaiaEDR3` | Radial velocity error from Gaia DR2; identical to `e_rv_gaia_dr2` | km&#8239;s<sup>-1</sup> | float |
...
| `phot_bp_mean_flux_error` | `VAC_GaiaEDR3` | Error on the integrated BP mean flux | electron&#8239;s<sup>-1</sup> | float |
...
| `phot_rp_mean_flux_error` | `VAC_GaiaEDR3` | Error on the integrated RP mean flux | electron&#8239;s<sup>-1</sup> | float |
